...
1. **BigQuery Setup**
   ...
   - Create a relevant BQ Dataset and tables in your project that stores the data to be queried
   - Ensure you have a proper Metadata file for the dataset tables that followw this format: [Sample Metadata file](https://github.com/GoogleCloudPlatform/nl2sql-studio/blob/main/UI/sample_metadata.json)

1. **Enable APIs**
   Ensure the Vertex AI APIs are enabled on the GCP project
...
To validate and ensure library dependencies are resolved, create a new Python env(preferably version 3.10 or above), activate the virtual env. Before running the below steps navigate to nl2sql_library folder in your local
...
**Alternatively, you can run the wrapper class in the nl2sql_library folder like so**
...
1. Starting the backend as a local service
   ...
   ```
   gunicorn --workers 1 --threads 8 --timeout 0 -b 0.0.0.0:8080 app:app
   ```
