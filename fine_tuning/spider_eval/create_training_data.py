                ...
                for i in range(start_index, len(spider_data), batch_size):
                    ...
                    for item, cot_result in zip(batch_items, generated_cots):
                        if isinstance(cot_result, Exception):
                            tqdm.write(f"Error generating CoT for question '{item['question']}': {cot_result}. Skipping this item.")
                            pbar.update(1)
                            continue
                        ...
                        tuning_record = create_record_func(
                            system_prompt, schema, item['question'], item['cot_reasoning'], item['query']
                        )
...
if __name__ == "__main__":
    INPUT_FILE_PATH = 'spider_data/train_final.json'
    
    MODEL_TYPE = "gemini" # possible values: "llama", "gemini", "qwen"
    GENERATE_COT = False
